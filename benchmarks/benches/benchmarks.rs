...
fn benchmark<T: DeserializeOwned + Serialize + From<usize> + for<'a> arrow_struct::FromArrayRef<'a>>(c: &mut Criterion, size: usize, name: &'static str) {
    let batch = setup_record_batch::<T>(size);
    let struct_array: StructArray = batch.clone().into();
    let array: ArrayRef = Arc::new(struct_array);
    c.bench_function(&format!("serde_arrow {}", name), |b| b.iter_with_large_drop(|| serde_arrow_convert::<T>(black_box(&batch))));
    c.bench_function(&format!("arrow_struct {}", name), |b| b.iter_with_large_drop(|| arrow_struct_convert::<T>(black_box(&array))));
}
